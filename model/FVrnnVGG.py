...
#         self.load_train_pretrain()
    ...
    def load_train_pretrain(self):
        if self.device == 'cuda':
            self.RNN.load_state_dict(torch.load('./storage/FVrnnVGG_RNN.pkl'))
            self.classifier.load_state_dict(torch.load('./storage/FVrnnVGG_classifier.pkl'))
        else :
            self.RNN.load_state_dict(torch.load('./storage/FVrnnVGG_RNN.pkl', map_location=lambda storage, loc: storage))
            self.classifier.load_state_dict(torch.load('./storage/FVrnnVGG_classifier.pkl', map_location=lambda storage, loc: storage))
        
        print("[INFO] load train pretrained weight successfully.")
            

def main():
    model = FVrnnVGG(backend='vgg13_bn', pretrained=False)
